module Finder
  def get_matching_keys(user_id, location, criteria)
    ...
    puts "keys == #{user_id}:patient:#{location}"
...
module Detail_finder 
  ...
  def find_details(user_id, criteria_string)
    matching_keys = get_matching_keys(user_id,"details",criteria_string)
    matching_values = []
...
module Reference_finder
  ...
  def find_reference(user_id, criteria_string)
    matching_keys = get_matching_keys(user_id, "reference", criteria_string)
    puts "reference matching_keys = #{matching_keys}"
    matching_values = []
    matching_keys.each do |key|
